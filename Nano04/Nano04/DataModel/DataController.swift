...
class DataController : ObservableObject {
    ...
    func addReminder(name: String, hoursToReset: Int, context: NSManagedObjectContext) {
        let reminders = Reminders (context: context)
        reminders.id = UUID()
        reminders.date = Date()
        reminders.hoursToReset = Int64(hoursToReset)
        reminders.name = name
//        reminders.toggle = Boolean()
    ...
    func editReminders(reminders: Reminders, name: String, hoursToReset: Int, date: Date = Date(), isDone: Bool, context: NSManagedObjectContext) {
           if isDone {
